...
if(isset($_POST['action']) && $_POST['action']=='save_sms'){
	$sms_text = urldecode($_POST['sms_text']);
	$sms_text = iconv("UTF-8", "CP1251", $sms_text);
	$phone = $_POST['phone'];
	// $phone = "0502952268";
	$answer = send_sms($phone, $sms_text, 0);
	$id = intval($_POST['id']);
	$zvonok = mysql_real_escape_string(iconv("UTF-8", "CP1251", $_POST['zvonok_text']));
	mysql_query("UPDATE `nb`.`clients` SET `zvonok` = '$zvonok' WHERE `clients`.`id` ='$id';");
	die();
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(".edit").on("click", function(){
				...
				$(".text_sms_button").on("click", function(){
					var this_id = $(this).data("id");
					$.post(document.location.href, {action: "save_sms", zvonok_text: $("#zvonok-text").val(), id: id, sms_text: $("#text_sms_"+this_id).val(), phone: phone})
						.success(function() {
							text.text($("#zvonok-text").val());
						});;
					$("#zvonok").dialog( "close" );
					$("#send_sms").dialog( "close" );
				});
			});
		});
	</script>
		...
		<tr>
			<td valign="top">
						...
						<tr>
							...
							<td>
								...
								<ul class="masters" style="list-style:none;">
									<li><a href="?page=0">1-500</a></li>
									<li><a href="?page=500">501-1000</a></li>
									<li><a href="?page=1000">1001-1500</a></li>
									<li><a href="?page=1500">1501-2000</a></li>
									<li><a href="?page=2000">2001-2500</a></li>
									<li><a href="?page=2500">2501-3000</a></li>
									<li><a href="?page=3000">3001-3500</a></li>
									<li><a href="?page=3500">3501-4000</a></li>
									<li><a href="?page=4000">4001-4500</a></li>
									<li><a href="?page=4500">4501-5000</a></li>
									<li><a href="?page=5000">5001-5500</a></li>
									<li><a href="?page=5500">5501-6000</a></li>
								</ul>
							</td>
						</tr>
					</table>
				...
				if(isset($_GET['page'])){
					...
					$resText="SELECT * FROM clients WHERE show1=1 ORDER BY id DESC LIMIT $page, 500";
				}elseif(isset($_GET['filter'])){
					$resText="SELECT * FROM clients WHERE show1=1 AND phone=".intval($_GET[filter])." ORDER BY id DESC LIMIT 0, 200";
				}else{
					$resText="SELECT * FROM clients WHERE show1=1 ORDER BY id DESC LIMIT 0, 500";
				}
				...
				?>
					<tr style="background:<?=$color?>;border:<?=$date_oplata?>">
						...
						<td><a class="phone col" href="?filter=<?=str_replace(" ","&nbsp; ",$row[phone])?>"><?=str_replace( " ", "&nbsp;",$row[phone])?></a></td>
...
<table>
	<tr>
		<td style="padding:5px 0;"><textarea style="width:380px;height:80px;margin:0;" disabled id="text_sms_1">Компьютерная помощь 0990541117 Вызов мастера бесплатно! Nanobyte.com.ua Павленко 15, ремонт ноутбука планшета, принтера, восст. данных</textarea></td>
		<td style="padding:10px;"><button data-id="1" class="text_sms_button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" style="padding:5px;">Отправить</button></td>
	</tr>
	<tr>
		<td style="padding:5px 0;"><textarea style="width:380px;height:80px;margin:0;" id="text_sms_4"></textarea></td>
		<td style="padding:10px;"><button data-id="4" class="text_sms_button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" style="padding:5px;">Отправить</button></td>
	</tr>
</table>
</div>
</body>

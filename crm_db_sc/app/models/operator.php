<?php
class app_models_operator extends app_model{
	public function get_list(){
		$sql = "SELECT ta.*, cl.*, de.*, fi.*, st.* FROM tasks ta\n"
				...
				."WHERE ta.ta_show = 1 AND ta.ta_st_id != 4 ORDER BY ta.ta_id DESC";
	...
	public function client_request($cl_id){
		$sql = "
			...
			LEFT JOIN find_out fi ON fi.fi_id = ta.ta_fi_id
			LEFT JOIN statuses st ON st.st_id = ta.ta_st_id
			WHERE ta.ta_show = 1 AND (cl.cl_id = ?i OR cl.cl_phone LIKE '%?e') ORDER BY ta.ta_id DESC
		";
		$list = $this->base()->query($sql, array($cl_id, $cl_id));
		$List = $list->assoc();
		foreach($List as $id => $item){
			$List[$id]['cl_phone'] = preg_replace ("/(\+7)(\d{3})(\d{3})(\d{2})(\d{2})/i","\$1(\$2) \$3-\$4-\$5",$item['cl_phone']);
		}
	...
	public function search($pattern){
		$sql = "
			...
			LEFT JOIN find_out fi ON fi.fi_id = ta.ta_fi_id
			LEFT JOIN statuses st ON st.st_id = ta.ta_st_id
			WHERE ta.ta_show = 1 AND (ta.ta_id = ?i OR cl.cl_phone LIKE '%?e' OR cl.cl_name LIKE '%?e%' OR ta.ta_model LIKE '%?e%') ORDER BY ta.ta_id DESC
		";
		...
		$list = $this->base()->query($sql, array($pattern, $pattern, $pattern, $pattern));
		$List = $list->assoc();
		foreach($List as $id => $item){
			$List[$id]['cl_phone'] = preg_replace ("/(\+7)(\d{3})(\d{3})(\d{2})(\d{2})/i","\$1(\$2) \$3-\$4-\$5",$item['cl_phone']);
		}
	...
	/*
	private function get_client($cl_id){
		$sql = "SELECT * FROM clients WHERE cl_id = ?i";
		$cl_id = $this->base()->query($sql, $cl_id, 'id');
		return $cl_id;
	}
	*/
}
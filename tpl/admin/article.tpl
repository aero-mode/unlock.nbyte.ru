...
<script>
	function del(id){
		if(confirm("Удалить?")){
			var answer = '';
			$.post('/admin-post/', { action: 'article_delete', id: id }, function(data){
				if(data == 'deleted'){
					$("#msg_ok").text("Удалено");
					$("#"+id).remove();
				}else{
...
<table>
	...
	<tr id="{{article.id}}">
		<td style="text-align:center;">
			<a href="{{article.edit_link}}"><img src="{{theme}}/img/ico_edit.gif" alt="Редактировать" title="Редактировать"></a>
			<a href="javascript:void(0)" onclick="return del({{article.id}});"><img src="{{theme}}/img/ico_del.gif" alt="Удалить" title="Удалить"></a>
		</td>
		<td>{{article.title}}</td>
	</tr>
	{% endfor %}
